...
Original file is located at
    https://colab.research.google.com/drive/10C_YguLxEago4LgNFwmNPCvlYUrZFkvg
"""
...
class FLAVORS2FeatureSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y,sample_weight):
        ...
        self.selector.fit(X, y,sample_weight)
        ...
        if self.selector.leaderboard and len(self.selector.leaderboard) > 0:
            ...
            self.selected_indices = best_subset
        else:
            self.selected_indices = list(range(X.shape[1]))  # fallback: all features

        unevaluated_indices = np.where(self.selector.feature_counts == 0)[0]
    ...
    def transform(self, X):
        ...
        if isinstance(X, pd.DataFrame):
            return pd.DataFrame(
                transformed,
                index=X.index,
                columns=[str(col) for col in self.get_feature_names_out(list(X.columns))]
            )
        else:
            return transformed

    def get_feature_names_out(self, input_features):
        """Given input feature names, return the selected feature names."""
        if self.selected_indices is None:
            return input_features  # fallback
        return [input_features[i] for i in self.selected_indices]
...
def time_consistency_r2_sliding_window(
    X: pd.DataFrame,
    y: pd.Series,
    window_size: int,
    time_unit: str = 'month', # New parameter
    min_samples: int = 5,
    model=LinearRegression()
) -> pd.DataFrame:
    """
    Evaluates R2 score for features using a sliding window approach,
    considering all tickers collectively, with purging for future 'y' labels.

    Args:
        X (pd.DataFrame): DataFrame of features with a MultiIndex of ('ticker', 'date').
        y (pd.Series): Series of target variable with a MultiIndex of ('ticker', 'date').
                        It's assumed that y[date] corresponds to a return 1 week after 'date'.
        window_size (int): The number of time_unit in each sliding window. The first time_unit
                            in the window is used for training, and the last time_unit for testing.
        time_unit (str): The unit of time for the sliding window. Can be 'month' or 'year'.
        min_samples (int): Minimum number of samples required for training and testing.

    Returns:
        pd.DataFrame: A DataFrame with R2 scores for each feature for each window.
                      Includes 'Feature', 'R2_Score', 'Train_Period', 'Test_Period', 'Is_Positive_R2'.
    """

    # Ensure the index has 'ticker' and 'date' levels and 'date' is datetime
    if not isinstance(X.index, pd.MultiIndex) or 'date' not in X.index.names or 'ticker' not in X.index.names:
        raise ValueError("X must have a MultiIndex with 'ticker' and 'date' levels.")

    if time_unit not in ['month', 'year']:
        raise ValueError("time_unit must be 'month' or 'year'.")

    # Convert the 'date' level to datetime objects if it's not already
    date_level_name = 'date'
    ticker_level_name = 'ticker'

    if X.index.get_level_values(date_level_name).dtype != '<M8[ns]':
        try:
            X = X.set_index(pd.MultiIndex.from_arrays([
                X.index.get_level_values(ticker_level_name),
                pd.to_datetime(X.index.get_level_values(date_level_name))
            ], names=X.index.names))
            y = y.set_axis(X.index)
            print("Info: 'date' index level converted to datetime type.")
        except Exception as e:
            raise ValueError(f"Could not convert 'date' index level to datetime. Error: {e}")

    # Ensure data is sorted by ticker, then date.
    X = X.sort_index(level=[ticker_level_name, date_level_name])
    y = y.reindex(X.index)

    all_results = []

    # Get unique sorted periods based on the specified time_unit
    if time_unit == 'month':
        unique_periods = X.index.get_level_values(date_level_name).to_period('M').unique().sort_values()
        period_frequency = 'M'
    else: # time_unit == 'year'
        unique_periods = X.index.get_level_values(date_level_name).to_period('Y').unique().sort_values()
        period_frequency = 'Y'


    # Check if enough periods exist for at least one window
    if len(unique_periods) < window_size:
        print(f"Warning: Not enough unique {time_unit}s ({len(unique_periods)}) in the data for window_size={window_size}. "
              f"Please provide data with more {time_unit}s.")
        return pd.DataFrame()

    # Create sliding windows of periods
    for i in range(len(unique_periods) - window_size + 1):
        window_periods = unique_periods[i : i + window_size]
        train_period = window_periods[0]
        test_period = window_periods[-1]

        # Calculate the start date of the test period to use for purging
        test_period_start_date = test_period.to_timestamp(how='start')

        # Filter X for the training period
        X_train_candidate = X[X.index.get_level_values(date_level_name).to_period(period_frequency) == train_period]

        # Filter y for the training period, ensuring future labels don't leak into the test period
        # A y label from original_date corresponds to a return at original_date + 1 week.
        # We need to ensure that (original_date + 1 week) < test_period_start_date
        y_train_mask = (y.index.get_level_values(date_level_name).to_period(period_frequency) == train_period) & \
                       ((y.index.get_level_values(date_level_name) + pd.Timedelta(weeks=1)) < test_period_start_date)
        y_train_candidate = y[y_train_mask]

        # Re-align X_train_candidate and y_train_candidate after purging on y's index
        # This ensures X and y have matching indices after potential purging
        common_train_indices = X_train_candidate.index.intersection(y_train_candidate.index)
        X_train_window = X_train_candidate.loc[common_train_indices]
        y_train_window = y_train_candidate.loc[common_train_indices]


        # Filter X and y for the test period
        X_test_window = X[X.index.get_level_values(date_level_name).to_period(period_frequency) == test_period]
        y_test_window = y[y.index.get_level_values(date_level_name).to_period(period_frequency) == test_period]

        if len(X_train_window) < min_samples or len(X_test_window) < min_samples:
            print(f"Skipping window {train_period}-{test_period}: insufficient samples (Train: {len(X_train_window)}, Test: {len(X_test_window)}) below min_samples={min_samples} after purging.")
            continue

        features = X.columns
        for feat in features:
            xtr = X_train_window[[feat]]
            xts = X_test_window[[feat]]

            # Skip features with no variance in either train or test set
            if xtr[feat].nunique() <= 1 or xts[feat].nunique() <= 1:
                continue

            try:
                model.fit(xtr, y_train_window)
    ...
    if all_results:
        df=pd.DataFrame(all_results).sort_values(["Train_Period", "R2_Score"], ascending=[True, False]).reset_index(drop=True)
        agg_df=df.groupby('Feature')['R2_Score'].agg(['std','mean','min','max'])
        return agg_df
    else:
        print("No features could be evaluated. Check data, window_size, and min_samples.")
        return pd.DataFrame()
...
def correlation_graph_filter(df, threshold=0.99):
    """
    Correlation graph-based feature pruning using both raw features
    and abs(x - median) transformed features.

    Parameters:
    - df: DataFrame of features
    - threshold: average correlation threshold to determine redundancy

    Returns:
    - features_to_keep: List of features kept
    - features_to_drop: List of features dropped
    """
    # Step 1: Create median-deviation transformed features
    meddev_df = df.apply(lambda x: np.abs(x - x.median()), axis=0)

    # Step 2: Compute two correlation matrices
    corr_raw = df.corr().abs()
    corr_meddev = meddev_df.corr().abs()

    # Step 3: Average the two correlation matrices
    avg_corr = 0.5 * (corr_raw + corr_meddev)
    ...
    for i in range(len(features)):
        for j in range(i + 1, len(features)):
            ...
            if avg_corr.loc[f1, f2] > threshold:
                G.add_edge(f1, f2)

    # Step 5: Greedy vertex cover
    features_to_drop = set()
    G_copy = G.copy()
    while G_copy.number_of_edges() > 0:
        # Remove feature with highest degree
        node = max(G_copy.degree, key=lambda x: x[1])[0]
        features_to_drop.add(node)
        G_copy.remove_node(node)

    features_to_keep = [f for f in features if f not in features_to_drop]
